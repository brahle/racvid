...
int main(int argc, char* argv[]) {
  ...
  vector<int> samples(config.getN());
  for (int i = 0; i < (int) samples.size(); ++i) {
    samples[i] = i;
  }
  random_shuffle(samples.begin(), samples.end());
  samples.resize(config.getNumberOfSamples());
  
  // files
  fstream fSamples(config.getSamples().c_str(), fstream::out);
  fstream fPeopleResults(config.getPeopleResults().c_str(), fstream::out);
  fstream fLuggageResults(config.getLuggageResults().c_str(), fstream::out);

  // headers
  fPeopleResults << "tp fp fn\n";
  fLuggageResults << "tp fp fn\n";
    
  // tp, fp, fn
  vector<int> tp[2], fp[2], fn[2];
  for (int i = 0; i < 2; ++i) {
    tp[i].resize(samples.size());
    fp[i].resize(samples.size());
    fn[i].resize(samples.size());
  }
  ...
  for (int i = 0; i < (int) samples.size(); ++i) {
    ...
    // tp, fp, fn
    for (int j = 0; j < 2; ++j) {
      cin >> tp[j][i] >> fp[j][i] >> fn[j][i];
    }
    fSamples << ii << "\n";
    fPeopleResults << tp[0][i] << " " << fp[0][i] << " " << fn[0][i] << "\n";
    fLuggageResults << tp[1][i] << " " << fp[1][i] << " " << fn[1][i] << "\n";
  ...
  // sum
  int sum_tp[2], sum_fp[2], sum_fn[2];
  for (int i = 0; i < 2; ++i) {
    sum_tp[i] = accumulate(tp[i].begin(), tp[i].end(), 0);
    sum_fp[i] = accumulate(fp[i].begin(), fp[i].end(), 0);
    sum_fn[i] = accumulate(fn[i].begin(), fn[i].end(), 0);
  }
  
  fPeopleResults << "\ntp fp fn\n";
  fPeopleResults << sum_tp[0] << " " << sum_fp[0] << " " << sum_fn[0] << "\n";
  
  fLuggageResults << "\ntp fp fn\n";
  fLuggageResults << sum_tp[1] << " " << sum_fp[1] << " " << sum_fn[1] << "\n";
  
  // measures
  double precision[2], recall[2];
  for (int i = 0; i < 2; ++i) {
    precision[i] = (double) sum_tp[i] / (sum_tp[i] + sum_fp[i]);
    recall[i] = (double) sum_tp[i] / (sum_tp[i] + sum_fn[i]);
  }
  
  fPeopleResults << "\nprecision recall\n";
  fPeopleResults << precision[0] << " " << recall[0] << "\n";
  
  fLuggageResults << "\nprecision recall\n";
  fLuggageResults << precision[1] << " " << recall[1] << "\n";
